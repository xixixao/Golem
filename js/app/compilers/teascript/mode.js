define(function (require, exports, module) {
  ...
  var BACKWARD, Behaviour, CombinedTooltip, CustomAutocomplete, CustomSearchBox, DistributingWorkerClient, EventEmitter, FIRST, FORWARD, HashHandler, LAST, NEXT, PREVIOUS, Range, Selection, TextMode, ace, all, ammendAst, ammendToken, ancestorAtDefinitonList, append, argumentNamesFromCall, astize, astizeExpressions, atomDelimiter, atomOrPositionFrom, atomReference, bookmarkBefore, changeNumericalAt, childIndex, childIndexOfTangible, cloneNode, cloneNodePreserving, cloneNodes, cloneNodesPreserving, compiler, concat, concatMap, concatTangibles, convertToAceLineTokens, convertToAceToken, definitionAncestorOf, depthOf, duplicateProperties, edgeIdxOfNode, edgeOfList, edgeWithinAtom, editableEdgeWithin, editableLength, escape, extend, fakeAtInsertion, filter, findAdjecentInList, findAllOccurences, findAllReferences, findFunctionBody, findNodeWithPosition, findNodesBetween, findOtherReferences, findParamList, findParentFunction, findParentScope, firstFakeInside, following, followingTangibleAtomOrPosition, inOrder, indexWithin, insToTangible, insertChildNodeAt, insideTangible, isAtom, isAtomOrPositionAt, isBetaReducible, isCall, isClosingDelim, isDelim, isDelimitedAtom, isEmptyCall, isExpression, isForm, isFunction, isHalfDelimitedAtom, isIndent, isLabel, isName, isNewLine, isNodeInside, isNumerical, isOpeningDelim, isOperator, isParentOfDefinitionList, isReal, isSpace, isWhitespace, join, limitTerm, listToForm, log, map, memorable, merge, next, nextExpression, nodeEdgeOfTangible, nodeEdgesOfTangible, nodesEqual, nodesToString, onlyExpression, oop, opposite, originOf, outsToTangible, padding, paddingEdge, parentOf, parentOfTangible, pickupBookmarks, positionsToRange, preceding, precedingWhitespace, previous, previousExpression, reindent, reindentMutateNodes, reindentNodes, reindentNodesPreserving, reindentTangible, reindentTangiblePreserving, repeat, setIndentTo, sibling, siblingAncestorsFrom, siblingLeaf, siblingTangibleAncestors, siblingTerm, sortSiblingTangibles, sortTuple, sortedArgs, spliceString, tangibleAncestor, tangibleBetween, tangibleEdge, tangibleInAfter, tangibleInside, tangibleMargin, tangibleParent, tangibleSurroundedBy, tangibleToIns, tangibleWithMargin, tangibleWithSomeMargin, termToTangible, toNode, toTangible, topList, unzip, validOut, zipWith, __, _arguments, _empty, _fst, _is, _labelName, _notEmpty, _operator, _ref, _snd, _symbol, _terms, _transformed, _validTerms,
...
exports.Mode = (function(_super) {
  ...
  function _Class(isSingleLineInput) {
    ...
    this.undoManager = __bind(this.undoManager, this);
  ...
  _Class.prototype.attachToEditor = function(editor) {
    ...
    session.setUndoManager(this.undoManager());
  ...
  _Class.prototype.updateAst = function(ast, errors) {
    ...
    return this.addErrorMarkers(errors);
  };
    ...
    if ((malformed = token.malformed) || (isDelim(token)) && (malformed = token.parent.malformed)) {
      ...
      return this.worker.call('docsFor', [reference], (function(_this) {
        return function(info) {
          if (info != null ? info.rawType : void 0) {
            return activate(token.label === 'name' ? _this.prettyPrintTypeForDoc(info) : _this.createDocTooltipHtml(info));
          }
        };
      })(this));
  ...
  _Class.prototype.createDocTooltipHtml = function(info) {
    var docs, paramNames, params;
    paramNames = info.arity || [];
    params = paramNames.join(' ');
    return ("<span style='color: #9EE062'>" + info.name + "</span> <span style='color: #9C49B6'>" + params + "</span>\n" + (this.prettyPrintTypeForDoc(info))) + (info.docs ? (docs = compiler.labelDocs(info.docs, paramNames), "\n" + docs) : '');
  };
  ...
  _Class.prototype.undoManager = function() {
    ...
    this.groupMutationRegister || (this.groupMutationRegister = {
      states: []
    });
    return {
      ...
      registerMutation: (function(_this) {
        return function(state, way) {
          var added, atom, from, replaced, reversals, stack, to, _ref1;
          stack = way.undo ? _this.undoStack : _this.redoStack;
          reversals = [];
          ...
          if (state.changeWithinAtom) {
            ...
            atom = state.changeWithinAtom.atom || _this.editedAtom();
            reversals.push({
              changeWithinAtom: {
          ...
          reversals.push(_this.isEditing() ? (atom = _this.editedAtom(), {
            withinAtom: atom,
            withinAtomPos: _this.distance(_this.startPos(atom), _this.editor.selection.getRange().end)
          }) : {
            tangibleSelection: _this.selectedTangible()
          });
          _this.groupMutationRegister.stack = stack;
          return _this.groupMutationRegister.states.push(merge(reversals));
        };
      })(this),
      packGroupMutation: (function(_this) {
        return function() {
          var res, stack, states, _ref1;
          _ref1 = _this.groupMutationRegister, stack = _ref1.stack, states = _ref1.states;
          states.reverse();
          stack.push((_this.sameKindMutation(states, stack[stack.length - 1] || []) ? res = states.concat(stack.pop()) : states));
          return _this.groupMutationRegister = {
            states: []
          };
        };
      })(this),
      undo: (function(_this) {
        return function() {
          return _this.replay(_this.undoStack, {
            redo: true
          });
        };
      })(this),
      redo: (function(_this) {
        return function() {
          return _this.replay(_this.redoStack, {
            undo: true
          });
        };
      })(this),
  ...
  _Class.prototype.sameKindMutation = function(previousStates, nextStates) {
    ...
    return this.editor.commands.addCommands(this.commands = {
          ...
          return function(editor, _arg) {
            ...
            if (targetEditor) {
              ...
              if (selected && selected.tea) {
                ...
                _this.mutate({
                  tangibleSelection: firstFakeInside(FORWARD, _this.onlySelectedExpression())
                });
      ...
      'wrap current selection in a Function': {
        ...
        exec: (function(_this) {
          return function() {
            var separator;
            separator = (parentOf(toNode(_this.selectedTangible()))) || _this.isSingleLineInput ? [' '] : ['\n', '  '];
            return _this.wrap.apply(_this, concat([
              ['(', 'fn', ' ', '[]'], separator, [
                {
                  selected: true,
                  select: true
      ...
      'Define selected token': {
        ...
        exec: (function(_this) {
          return function(editor, _arg) {
            var args, atom, definition, hole, moved, movedTo, newHole, originalHole, originalHoles, rememberedHoles, selected, selection, selections, separator, targetEditor, targetMode, top;
            targetEditor = (_arg != null ? _arg : {}).targetEditor;
            ...
            targetMode = targetEditor.session.getMode();
            selected = targetMode.onlySelectedExpression();
            if (selected && (isAtom(atom = selected)) && atom.malformed) {
              _this.startGroupMutation();
              if (targetEditor !== _this.editor) {
              ...
              if (targetEditor !== _this.editor) {
                moved = reindentTangible(toTangible(selected), _this.parentOfSelected());
                _this.startGroupMutation();
                originalHole = bookmarkBefore(_this.selectedTangible());
                ...
                top = ancestorAtDefinitonList(selected);
                movedTo = nodeEdgeOfTangible(LAST, toTangible(top));
                moved = reindentTangible(_this.selectedTangible(), top.parent);
                separator = parentOf(top) ? '\n' : '\n\n';
                _this.startGroupMutation();
                selections = _this.selectedTangiblesList();
                ...
                _this.insertSpaceAt(FORWARD, separator, movedTo);
                newHole = bookmarkBefore(_this.selectedTangible());
                _this.insertSpaceAt(FORWARD, " ", movedTo);
                ...
                _this.mutate({
                  ...
                  newSelections: join(rememberedHoles.slice(1), [newHole()])
                });
                return _this.finishGroupMutation();
              }
            }
      ...
      'Inline selected expression or replace a name by its definition': {
        ...
        exec: (function(_this) {
          return function() {
            var arg, args, atom, body, definition, fn, fun, i, indented, inlined, name, numInlined, other, others, paramIndex, paramList, params, reducible, reindentedBody, reindentedFn, selected, use, uses, _i, _j, _k, _len, _len1, _len2;
            selected = _this.onlySelectedExpression();
            if (selected && (isAtom(atom = selected))) {
              others = (findOtherReferences(atom))(_this.ast);
              if (isName(atom)) {
  ...
  _Class.prototype.wrap = function() {
    ...
    return this.wrapIn(string, this.selectedTangible(), tags);
  };

  _Class.prototype.wrapIn = function(wrapperString, tangible, _arg) {
    var empty, insert, select, selected, selections, wrapped, wrapper;
  ...
  _Class.prototype.mutate = function(state, way) {
    var added, addedString, atom, editing, range, recover, removedRange, replaced, selectionNodes, selectionRange, selections, tangible, _i, _len, _ref1, _ref2;
    if (way == null) {
      way = {
        undo: true
      };
    }
    this.undoManager().registerMutation(state, way);
    if (!this.groupMutating) {
      this.undoManager().packGroupMutation();
    }
    if (state.changeInTree) {
      replaced = state.changeInTree.at;
      added = state.changeInTree.added || [];
  ...
  _Class.prototype.reportModuleName = function(moduleName) {
...
firstFakeInside = function(direction, form) {
  var child, inside, notFake;
  inside = tangibleInside(opposite(direction), form);
  while ((notFake = isExpression(child = toNode(inside))) && isNodeInside(child, form)) {
...
convertToAceToken = function(token) {
  return {
    ...
    type: ((function() {
      if ((isDelim(token)) && token.parent.malformed || token.malformed) {
