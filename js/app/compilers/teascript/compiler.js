...
Context = (function() {
  ...
  Context.prototype.req = function(moduleName, names) {
    ...
    if (!inSet(this.importedModules, moduleName)) {
  ...
  Context.prototype.declareMacro = function(name, macro) {
...
callMacroCompile = function(ctx, call) {
  var compiled, expanded, macro, op;
  op = _operator(call);
  op.label = 'keyword';
...
callJsMethodCompile = function(ctx, call) {
  var args, dotMethod, object, _ref;
  _ref = _validTerms(call), dotMethod = _ref[0], object = _ref[1], args = 3 <= _ref.length ? __slice.call(_ref, 2) : [];
  labelOperator(dotMethod);
  call.tea = toConstrained(markOrigin(jsType, call));
  return assignCompile(ctx, call, object ? /^.-/.test(dotMethod.symbol) ? jsAccess(termCompile(ctx, object), dotMethod.symbol.slice(2)) : jsMethod(termCompile(ctx, object), dotMethod.symbol.slice(1), termsCompile(ctx, args)) : (malformed(ctx, call, 'Missing an object'), jsNoop()));
};
...
ms.match = ms_match = function(ctx, call) {
  ...
  compiledCases = conditional((function() {
    ...
    return _results;
  })(), "throw new Error('match failed to match " + (print(subject)) + errorMessage + "');");
...
ms.req = ms_req = function(ctx, call) {
  var moduleName, reqTuple, reqs, requiredNames;
  ...
  moduleName = _validArguments(call)[0];
  if (!moduleName || !isName(moduleName)) {
  ...
  return imports(moduleName.symbol, setToArray(requiredNames));
};
...
ms.syntax = ms_syntax = function(ctx, call) {
  var compiledMacro, doDeclare, docs, hasName, macroCompiled, macroFn, macroName, macroSource, maybeType, param, paramTuple, params, rest, splatted, _ref;
  ...
  if (hasName) {
    ...
    if (macroFn) {
      ctx.declareMacro(ctx.definitionPattern(), function(ctx, call) {
        if (splatted) {
          return constantToSource(macroFn(Immutable.List(_arguments(call))));
        } else {
          return constantToSource(macroFn.apply(null, _arguments(call)));
        }
      });
    } else {
...
firstOrCall = function(args) {
  if (args.length > 1) {
    return call_(args[0], args.slice(1));
  } else {
    return args[0];
  }
};
...
validIdentifier = function(name) {
  return (inSet(reservedInJs, name) ? "" + name + "_" : name === '.' ? "dot_" : name).replace(/\+/g, 'plus_').replace(/\-/g, '__').replace(/\*/g, 'times_').replace(/\//g, 'over_').replace(/\!/g, 'not_').replace(/\=/g, 'eq_').replace(/\</g, 'lt_').replace(/\>/g, 'gt_').replace(/\~/g, 'neg_').replace(/\^/g, 'pow_').replace(/\&/g, 'and_').replace(/\@/g, 'at_').replace(/\|/g, 'or_').replace(/\?/g, 'p_');
};
...
compileTopLevel = function(source, moduleName, requiredMap) {
  var ast, compilationFn, ctx, defaultImports, errors, ir, js, request, requiredModuleName, toInject, _ref1;
  if (moduleName == null) {
    moduleName = '@unnamed';
  }
  if (requiredMap == null) {
  ...
  _ref1 = compileCtxAstToJs(compilationFn, ctx, astFromSource("(" + source + ")", -1, -1)), request = _ref1.request, ast = _ref1.ast, ir = _ref1.ir, js = _ref1.js;
  if (request) {
...
compileModule = function(moduleName) {
  if (moduleName == null) {
    moduleName = '@unnamed';
  }
  return {
...
compileExpression = function(source, moduleName) {
  var ast, compilationFn, ctx, errors, expression, js, module, modules, _ref1;
  if (moduleName == null) {
    moduleName = '@unnamed';
  }
  ...
  if (_empty(_validTerms(ast))) {
    ...
    js = compileCtxAstToJs(compilationFn, ctx, expression).js;
    errors = checkTypes(ctx);
...
lookupJs = function(moduleName) {
  var js, _ref1;
  js = (_ref1 = lookupCompiledModule(moduleName)) != null ? _ref1.js : void 0;
  if (!js) {
...
injectContext = function(ctx, compiledModule, moduleName, names) {
  var arity, classes, definitions, docs, final, isClass, macro, macros, name, shouldImport, topScope, type, typeNames, virtual, _ref1, _ref2, _ref3;
  ...
  for (name in _ref2) {
    _ref3 = _ref2[name], type = _ref3.type, arity = _ref3.arity, docs = _ref3.docs, isClass = _ref3.isClass, virtual = _ref3.virtual, final = _ref3.final;
    if (shouldImport(name)) {
...
findDocsFor = function(moduleName, reference) {
  var arity, ctx, docs, found, name, savedScope, savedScopes, scope, type;
  savedScopes = lookupCompiledModule(moduleName).declared.savedScopes;
  ctx = contextWithDependencies(reverseModuleDependencies(moduleName)).ctx;
  ...
  found || (found = lookupInMap(ctx._scope(), name));
  if (found) {
    arity = found.arity, type = found.type, docs = found.docs;
    return {
      name: name,
      rawType: type,
      docs: docs,
