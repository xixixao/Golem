define(function (require, exports, module) {
  ...
  var CommandMode, CommandParser, EditorMain, History, Memory, Mode, Modes, SetIntervalMixin, TimeLine, ace, helpDescription, hyper, jsDump, _AdjustableColumns, _CommandLine, _FillHeight, _MessageDisplay, _OutputDisplay, _SourceEditor, _div, _input, _ref,
  __slice = [].slice,
...
module.exports = hyper(EditorMain = (function() {
  ...
  EditorMain.prototype.getInitialState = function() {
    ...
    this.saved = false;
    this.unnamedModules = 0;
    mode = new Mode(false, this);
  ...
  EditorMain.prototype.unnamedModuleName = function() {
    var newModuleName;
    newModuleName = "unnamed" + (this.unnamedModules++);
    this.state.mode.worker.call('compileModule', ['', newModuleName]);
    return newModuleName;
  };

  EditorMain.prototype.createAceEditor = function(value, displayId, moduleName, isTopLevel) {
    var command, container, editor, mode, name, notFocusedClass, _ref1;
    ...
    editor.on('blur', function() {
      return container.classList.add(notFocusedClass);
    });
    editor.on('focus', function() {
      return container.classList.remove(notFocusedClass);
    });
    mode.registerWithWorker(this.state.mode.worker);
      ...
      if (command.indirect) {
        (function(_this) {
          return (function(name, command) {
            return command.exec = function() {
              return _this.handleExpressionCommand(name, editor);
            };
          });
        })(this)(name, command);
      }
    }
    return mode.worker.on('ok', (function(_this) {
      return function(_arg) {
        var result, source, type, _ref2;
        _ref2 = _arg.data, result = _ref2.result, type = _ref2.type;
  ...
  EditorMain.prototype.handleRemoveAll = function() {
    return this.executeCommand('erase');
  };
  ...
  EditorMain.prototype.componentDidMount = function() {
    if (this.state.timeline.size() < 10) {
      this._executeCommand(this.state.commandMap['intro']);
    }
  ...
  EditorMain.prototype.handleOutputDelete = function(deletedId) {
